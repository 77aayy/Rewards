 ...
 *
 * مهم: التعديلات المحلية لا تظهر على الموقع المنشور حتى تنشرها.
 * بعد تعديل الكود: نفّذ npm run build ثم npm run deploy ثم شغّل الاختبارات.
 * أو شغّل محلياً: npm run build && npm run start ثم في طرفية أخرى:
 *   set E2E_BASE_URL=http://localhost:8080 && npx playwright test e2e/e2e-full.spec.js --project=chromium
 *
...
test.describe('E2E Full Checklist', () => {
  ...
  test('9. إغلاق الفترة ثم التحقق من ظهورها في الأرشيف', async ({ page }) => {
    await page.goto(ADMIN_URL, GOTO_OPTS);
    await expect(page.locator('#dashboard')).toBeVisible({ timeout: 20000 });
    ...
    await expect(page.locator('body')).toContainText('تم إغلاق الفترة بنجاح', { timeout: 15000 });
    await expect(page.locator('#uploadBox')).toBeVisible({ timeout: 10000 });
