 ...
 * على Windows بدون tee: نفس الأمر أو | Out-File e2e-run.txt
 */
...
test.describe('E2E Full Checklist', () => {
  ...
  test('2. رابط المشرف يفتح على متصفح موظف جديد (بدون جلسة سابقة)', async ({ browser }) => {
    ...
    await adminPage.goto(ADMIN_URL, { waitUntil: 'networkidle', timeout: 15000 });
    ...
    await expect(adminPage.locator('#adminLink_supervisor')).toBeVisible({ timeout: 8000 });
    ...
    await supervisorPage.goto(supervisorHref, { waitUntil: 'networkidle', timeout: 25000 });
    ...
    // يجب أن تظهر لوحة المشرف والجدول
    await expect(supervisorPage.locator('body')).toContainText('المشرف', { timeout: 5000 });
    await expect(supervisorPage.locator('#mainTable')).toBeVisible({ timeout: 5000 });
    await expect(supervisorPage.locator('#dashboard')).toBeVisible({ timeout: 3000 });
    ...
    await expect(supervisorPage.locator('text=تم الإرسال بنجاح')).toBeVisible({ timeout: 8000 });
  ...
  test('4. رابط HR يفتح على متصفح موظف جديد (بدون جلسة سابقة)', async ({ browser }) => {
    ...
    await expect(adminPage.locator('#dashboard')).toBeVisible({ timeout: 15000 });
    await adminPage.locator('button:has-text("إدارة الإداريين")').click();
    await expect(adminPage.locator('#adminLink_hr')).toBeVisible({ timeout: 8000 });
    ...
    await hrPage.goto(hrHref, { waitUntil: 'networkidle', timeout: 25000 });

    const errorMsg = hrPage.getByText('تعذّر تحميل بيانات الفترة');
    await expect(errorMsg).not.toBeVisible({ timeout: PERIOD_LOAD_TIMEOUT });

    await expect(hrPage.locator('body')).toContainText('HR', { timeout: 5000 });
    await expect(hrPage.locator('#mainTable')).toBeVisible({ timeout: 5000 });
    ...
    await expect(hrPage.locator('text=تم الإرسال بنجاح')).toBeVisible({ timeout: 8000 });
    await employeeContext.close();
  });
  ...
  test('5. التحقق عند الأدمن (متصفح جديد) من ظهور التقييمات والحضور من Firebase', async ({ page }) => {
    ...
    await expect(table.locator('td.col-eval-booking')).toContainText('5', { timeout: 8000 });
    await expect(table.locator('td.col-eval-google')).toContainText('3', { timeout: 5000 });
    await expect(table.locator('td.col-attendance')).toContainText(/نعم|✓|1/, { timeout: 5000 });
  });
  ...
  test('6. روابط المدير العام والحسابات تفتح على متصفح موظف جديد', async ({ browser }) => {
    ...
    await expect(adminPage.locator('#dashboard')).toBeVisible({ timeout: 15000 });
    await adminPage.locator('button:has-text("إدارة الإداريين")').click();
    await expect(adminPage.locator('#adminLink_manager')).toBeVisible({ timeout: 5000 });
    ...
    await managerPage.goto(managerHref, { waitUntil: 'networkidle', timeout: 25000 });
    await expect(managerPage.getByText('تعذّر تحميل بيانات الفترة')).not.toBeVisible({ timeout: PERIOD_LOAD_TIMEOUT });
    await expect(managerPage.locator('body')).toContainText('المدير', { timeout: 5000 });
    await expect(managerPage.locator('#mainTable')).toBeVisible({ timeout: 5000 });
    await managerPage.close();

    const accountingPage = await empContext.newPage();
    await accountingPage.goto(accountingHref, { waitUntil: 'networkidle', timeout: 25000 });
    await expect(accountingPage.getByText('تعذّر تحميل بيانات الفترة')).not.toBeVisible({ timeout: PERIOD_LOAD_TIMEOUT });
    await expect(accountingPage.locator('body')).toContainText('الحسابات', { timeout: 5000 });
    await expect(accountingPage.locator('#mainTable')).toBeVisible({ timeout: 5000 });
